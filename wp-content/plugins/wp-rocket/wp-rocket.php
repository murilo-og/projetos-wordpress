...
$consumer_data = [
    ...
    'consumer_key'   => '10800200',
    'consumer_email' => 'validemail@example.com',
    'secret_key'     => hash( 'crc32', 'validemail@example.com' ),
];
update_option( 'wp_rocket_settings', array_merge( get_option( 'wp_rocket_settings', [] ), $consumer_data ) );
add_filter( 'pre_http_request', function( $pre, $parsed_args, $url ) use ( $consumer_data ) {
    if ( strpos( $url, 'https://wp-rocket.me/valid_key.php' ) !== false || strpos( $url, 'https://api.wp-rocket.me/api/wp-rocket/activate-licence.php' ) !== false ) {
        return [
            'response' => [
                'code'    => 200,
                'message' => 'OK'
	...
	if ( strpos( $url, 'https://api.wp-rocket.me/stat/1.0/wp-rocket/user.php' ) !== false ) {
		set_transient( 'wp_rocket_customer_data', $consumer_data, DAY_IN_SECONDS );
		return [
...
add_filter( 'rocket_valid_key', '__return_true', 100 );


// Rocket defines.
define( 'WP_ROCKET_VERSION',               '3.18.3' );
